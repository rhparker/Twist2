...
&i\partial_z c_n + \partial_t^2 c_n + k\left(e^{i\phi}c_{n-1}+e^{-i\phi}c_{n+1}\right)+|c_n|^2 c_n = 0.
\end{align}
The complex amplitudes $c_n$ are now functions of both $z$ and $t$. Using asymptotic analysis, we derive leading order expressions for solutions in which the bulk of the intensity is contained in a single core. We demonstrate the optical suppression also occurs in this model, to leading order, when the number of cores $N$ and phase parameter $\phi$ are related by $N=\pi/N$. This paper is organized as follows. [SECTION SUMMARY].
...
\end{align}
where the subscripts $n$ are taken $\md N$. When the coupling parameter $k=0$, which is known as the anti-continuum (AC) limit, this reduces to $N$ independent copies of the standing wave equation for the NLS equation. The solution at each site is either 0 or the NLS soliton
...
\end{align*}
Substituting \cref{eq:tildeanpsi} for the $\tilde{a}_n(t)$ and simplifying, we can again write everything in terms of $\psi(t)$:
\begin{equation}\label{eq:tildeantnpsi}
\begin{aligned}
\tilde{a}_{N/2-1}(t) \tilde{\theta}_{N/2-1}(t) &= -\sin(N \phi) (\omega - \partial_t^2)^{-(N/2+1)} \psi(t)  \\
\tilde{a}_n(t) \tilde{\theta}_n(t) &= -\sin(N \phi) (\omega - \partial_t^2)^{-(N-n)} \psi(t) && n = 1, \dots, N/2-2.
\end{aligned}
\end{equation}
