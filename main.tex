...
\textbf{Keywords.} nonlinear optics, topological photonics, nonlinear Schr\"odinger equation, twisted multi-core fibers, spatio-temporal localization
...
For all simulations, unless otherwise indicated, we will take $N=6$ fibers and $\omega=1$. First, we construct pulse solutions to \cref{eq:standingwave} by using parameter continuation from the anti-continuum (AC) limit ($k=0$). To do this, we take $C_n = u_n + i v_n$ in \cref{eq:standingwave}, and separate real and imaginary parts to obtain
